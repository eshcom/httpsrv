		...
		{tcp, _, Request} ->
			...
			Response = case string:prefix(Query, "/?file=") of
							...
							FileName ->
								...
								FilePath = lists:concat([CurrDir, "/data/", FileName]),
								%~ io:format("File path: ~ts~n", [FilePath]),
								case catch read_file(FilePath, ?FILE_READ_TIMEOUT) of
									ok ->
										http_200("File read successfully.");
									{error_open, _} ->
										http_404("File not found.");
									{error_read, _} ->
										http_503("File read error.");
